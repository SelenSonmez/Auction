...
public class DatabaseConnection {
    ...
    public static void addBidToList(Bid bid){
        ...
        if(bid.getBuyer() instanceof Millers){
            query += "'"+((Millers) bid.getBuyer()).getID()+"')";
        }
    ...
    public static ArrayList<Product> getProductsToList(){
        ...
        String query = "SELECT * FROM Products WHERE AUCTIONID = "+SessionSelection.ID;
        String name = null;
        double price = 0;
        int id = 0;
        Seller seller = null;
    ...
    public static void addProductToList(Product product){
        String query = "INSERT INTO Products(NAME,PRICE,AUCTIONID,PRODUCTSELLERID) VALUES("+"'"+product.getName()+"',"+product.getPrice()+","+SessionSelection.ID+",";
        ...
        if(product.getSeller() instanceof Millers){
            query+= "'"+((Millers)product.getSeller()).getID()+"')";
        }
        try{
            con = dataSource.getConnection();
            stmt = con.createStatement();
    ...
    public static User getUser(int ID, String password){
        ...
        try{
            ...
            switch(type){
                ...
                case "Miller" ->{
                    return new Millers(rs.getInt("ID"),rs.getString("USERNAME"),rs.getString("SURNAME"),
                            rs.getString("PASSWORD"),rs.getLong("CONTACTNO"),rs.getString("ADDRESS"));
                }
                case "Farming Agency" ->{
